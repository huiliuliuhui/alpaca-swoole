...
use Classes\Config;
use Classes\Db;
use Classes\Db\Builder;
use Classes\Db\Connection;
use Classes\Db\exception\BindParamException;
use Classes\Db\exception\DataNotFoundException;
use Classes\Db\exception\ModelNotFoundException;
use Classes\Exception;
use Classes\Exception\DbException;
use Classes\Exception\PDOException;
use Classes\Loader;
use Classes\Model;
use Classes\Model\Relation;
...
class Query
{
    ...
    public function getTable($name = '')
    {
        if ($name || empty($this->table)) {
            ...
            if ($name) {
                $tableName .= $name; //Loader::parseName($name);
            }
        } else {
     ...
     * @throws PDOException
     */
    public function queryResultArray($sql, $bind = [], $master = false, $class = false)
    {
        $result = $this->query($sql, $bind, $master, $class);

        return Util::objectArray2array($result);
    }
    ...
    protected function builder()
    {
        ...
        if (!isset($builder[$driver])) {
            $class            = false !== strpos($driver, '\\') ? $driver : '\\Classes\\Db\\Builder\\' . ucfirst($driver);
    ...
    /**
     * 批量添加，构造values语句
     * @param array $arrObj 对象组成的数组
     * @param bool $replace
     * @param string $table
     * @return bool
     */
    public function insertBatch($arrObj,$replace = false,$table = '')
    {
        if (!is_array($arrObj)) {
//            Public_Log::fatal("批量插入数据失败，数据格式错误");
//            Public_Log::fatal("操作数据:" . var_export($arrObj, true));
            return false;
        }
        ...
        $insert_field = array_intersect(array_keys(current($arrObj)), $fields);
        if (empty($insert_field)) {
//            Public_Log::fatal("操作数据:" . var_export($arrObj, true) . '  table fields:' . var_export($this->_fields, true));
//            Public_Log::fatal("批量插入数据失败，获取插入field失败");
            return false;
        }
        ...
        foreach ($arrObj as $obj) {
            // 按照插入域的顺序获取数据
            foreach ($insert_field as $field) {
                if (!array_key_exists($field, $obj)) {
//                    $err = "批量插入数据失败，插入对象字段不一致" . var_export($field, true) . var_export($obj, true);
//                    Public_Log::fatal($err);
                    return false;
                }
            ...
            $insert_value = substr($insert_value, 0, strlen($insert_value) - 2) . "), (";
        }

        $insert_value = substr($insert_value, 0, strlen($insert_value) - 3);

        $fields = join(",", $insert_field);
        if ($replace) {
            $sql = "replace into ";
        } else {
            $sql = "insert into ";
        ...
        $sql .= " $table($fields) values $insert_value";

        $ret = $this->query($sql);

        //Public_Log::notice("DaoBase执行的SQL:**{" . $this->_db->getLastSQL() . "}**");
        if ($ret === false) {
//            Public_Log::fatal("批量插入数据失败：" . $this->_db->getLastSQL());
            return false;
        }
    ...
    {
        $out = [];
        //字段没有设置,表名已经设置,那么自动获取所有的列.
        $sql = "show columns from $table";
        $s = $this->query($sql);
        foreach ($s as $fieldInfo) {
    ...
    {
        return $this->connection->startLog();
    }
    ...
    {
        return $this->connection->closeLog();
    }
