...
abstract class Connection
{
    ...
    //新增获取SQL集和耗时时间集
    protected $start_sql_log=0;//记录开启和关闭的SQL集
    protected $sql_log=array('sql_log'=>[],'time'=>[]);
    ...
    // 查询结果类型
    protected $fetchType = PDO::FETCH_OBJ; //PDO::FETCH_ASSOC;
    // 字段属性大小写
    ...
    public function query($sql, $bind = [], $master = false, $class = false)
    {
        ...
        if($this->start_sql_log){
            $this->sql_log['sql_log'][]=$this->queryStr;
        }
        ...
        try {
            ...
            //记录运行时间
            if($this->start_sql_log){
                $this->sql_log['time'][]=round($time_end-$time_start,4);
            }
            // 调试结束
            $this->debug(false);
    ...
    {
        $this->start_sql_log=1;
        $this->sql_log=array('sql_log'=>[],'time'=>[]);
    }
