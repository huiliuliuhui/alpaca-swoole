...
class Log
{
    ...
    public static function init($config = [])
    {
        ...
        $class        = false !== strpos($type, '\\') ? $type : '\\Classes\\Log\\Driver\\' . ucwords($type);
        self::$config = $config;
        unset($config['type']);
        if (class_exists($class)) {
            self::$driver = new $class($config);
        } else {
            throw new \Yaf\Exception('class not exists:' . $class, $class);
        }
        // 记录初始化信息
    ...
    public static function save()
    {
        if (!empty(self::$log)) {
            if (is_null(self::$driver)) {
				$config = \Yaf\Application::app()->getConfig()->log->toArray();
                self::init($config);
            }
    ...
    public static function __callStatic($method, $args)
    {
        if (in_array($method, self::$type)) {
            ...
            return call_user_func_array('\\Classes\\Log::record', $args);
        }
    }
